...
Use this for fast inspection. Move to the tool-specific sections below when the user needs layout-aware extraction, OCR, forms, rendering, or creation.
...
**Text extraction:** `pdftotext` is the fastest option for plain text. Use pdfplumber when the user needs tables or coordinate data — don't use `pypdf.extract_text()` on large documents, it's slow.

**Image extraction:** `pdfimages` extracts embedded images directly and is much faster than rendering whole pages. Only render with pypdfium2 or pdftoppm when the user needs a visual snapshot of the page layout.
