...
Reduce response size by selecting only the fields the user needs:
...
- **Use cursor paging** (`cursor=*`) when the user needs more than 10,000 results
...
- **Filter by `has_abstract:true`** when the user needs abstracts (not all works have them)
