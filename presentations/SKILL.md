---
name: presentations
description: "Create, manage, validate, preview, and export HTML presentation slides (1920x1080). Load this skill when the user needs to build a slide deck, export to PDF/PPTX, or preview slides in a browser."
---
...
The viewer is **not** a persistent background service. Start it on-demand with the `serve` action when the user needs to preview slides:

```bash
bun run "$SCRIPT" serve '{"port":3210}'
```

This starts a Bun server on port 3210 that serves all presentations under the `presentations/` directory. When the user needs it to keep running, launch the same command in `pty_spawn`.
